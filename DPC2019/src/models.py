...
TRAIN_PATH = '/home/dgxuser112/data/team2/ICIAR2018/dataset/train'
VALIDATION_PATH = '/home/dgxuser112/data/team2/ICIAR2018/dataset/validation'
...
class PatchWiseModel(BaseModel):
    ...
    def train(self):
        ...
        train_loader = DataLoader(
            dataset=PatchWiseDataset(path=TRAIN_PATH, stride=self.args.patch_stride, rotate=True, flip=True, enhance=True),
    ...
    def validate(self, verbose=True):
        ...
        test_loader = DataLoader(
            dataset=PatchWiseDataset(path=VALIDATION_PATH, stride=self.args.patch_stride),
            batch_size=self.args.batch_size,
            shuffle=False,num_workers=4
        )
        if verbose:
...
class ImageWiseModel(BaseModel):
    ...
    def train(self):
        ...
        train_loader = self._patch_loader(TRAIN_PATH, True)
    ...
    def validate(self, verbose=True, roc=False):
        ...
        if self._test_loader is None:
            self._test_loader = self._patch_loader(VALIDATION_PATH, False)
    ...
    def _patch_loader(self, path, augment):
        ...
        if self.args.debug and augment and os.path.exists(images_path):
            ...
            bsize = 1
            output_loader = DataLoader(dataset=dataset, batch_size=bsize, shuffle=False, num_workers=0)
            output_images = []
            output_labels = []
            ...
            output_images = output_images.astype('float64', copy=True)
            output_labels = output_labels.astype('float64', copy=True)
